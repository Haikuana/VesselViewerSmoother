...
#define GRD_orthogonal//do not use inclined grids
...
public:
	...
	surface* calculate_isosurface(MC33_real iso); // creates an iso-surface
	MC33(void);
	~MC33(void);
};
...
inline void _multTSA_bf(const double (*A)[3], MC33_real *b, MC33_real *c, int t)
{
	if(t)
	{
...
inline void _multA_bf(const double (*A)[3], MC33_real* b, MC33_real* c, int t)
{
	double u,v;
	if(t)
	{
